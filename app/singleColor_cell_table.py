...
def extract_imaging_and_injection_data(ptrsb):
    with engine.connect() as connection:
        ...
        print('injection_df: ', injection_df)
        # 确保 'PTRS(B)' 列的数据类型一致
        ...
        print('merged_df2222222: ', merged_df)
        return merged_df
def extract_imaging_columns(df):
    ...
    #     # 确保是DataFrame类型
    #     df = pd.DataFrame(df)
    print('extract_imaging_columns: ', df)
...
def extract_injection_columns(df):
    ...
    #     # 确保是DataFrame类型
    #     df = pd.DataFrame(df)
    print('extract_injection_columns: ', df)
...
def generate_cell_csv(extracted_df,output_path):
    ...
    def compute_post_perfusion_4percent_pfa(row):
        if str(row['fresh_perfusion']) == '0':
            return 1
        elif str(row['fresh_perfusion']) == '1':
            return 0
        else:
            return ''
