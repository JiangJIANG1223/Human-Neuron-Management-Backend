...
class ImagingInfo:
    ...
    def _parse_xlsx(self, imaging_path):
        ...
        for row in sheet.iter_rows(values_only=True):
            ...
            # 解析激光波长 (Laser Wavelength)
            if 'Laser' in row_str and 'nm' in row_str:
                laser_match_result = re.search(r'Laser ([0-9]+) nm', row_str)
                # print("laser_wavelength: ", laser_match_result)
                if laser_match_result:
                    self.laser_wavelength = laser_match_result.group(1)

            # 解析激光功率比率 (Laser Power Ratio)
            elif 'Power:' in row_str:
                power_match_result = re.search(r'Power:\s*([0-9.]+)', row_str)
                # print("laser_power_ratio: ", power_match_result)
                if power_match_result:
                    self.laser_power_ratio = power_match_result.group(1)

            # 解析增益 (Gain)
            elif 'Gain' in row_str:
                gain_match_result = re.search(r'Gain:\s*([0-9.]+)', row_str)
                # print("gain: ", gain_match_result)
                if gain_match_result:
                    self.gain = gain_match_result.group(1)

            # 解析扫描器 (Scanner)
            elif 'Scanner' in row_str:
                scanner_match_result = re.search(r'Scanner:\s*(.*)', row_str)
                # print("scanner: ", scanner_match_result)
                if scanner_match_result:
                    self.scanner = scanner_match_result.group(1)

            # 解析平均次数 (Line Averaging)
            elif 'Line Averaging' in row_str:
                averaging_match_result = re.search(r'Line Averaging:\s*([0-9]+)x', row_str)
                # print("averaging: ", averaging_match_result)
                if averaging_match_result:
                    self.averaging = averaging_match_result.group(1)

            # 基于 "Calibration" 字段 解析XY轴分辨率 (XY Resolution) 
            elif 'Calibration' in row_str:
                # 使用更通用的正则表达式匹配格式
            ...
            # 解析图像尺寸 (Image Size)
            elif 'Resolution' in row_str:
                resolution_match_result = re.search(r'Resolution:\s*([0-9]+)\s*x\s*([0-9]+)', row_str)
                # print("image_size: ", resolution_match_result)
                if resolution_match_result:
                    self.image_size = resolution_match_result.group(1) + '*' + resolution_match_result.group(2)
...
def process_single_file_pair(apo_file_path, metadata_file_path):
    """
    处理单个.apo和元数据文件对，而非批量处理

    参数:
        apo_file_path: .apo文件的完整路径
        metadata_file_path: 元数据文件(.xlsx或.xml)的完整路径
...
if __name__ == "__main__":
    ...
    # apo_dir = '/mnt/nfs/hndb/Imaging_Files/Single_Color_Somas'
...
# def count_data_rows_in_apo(file_path):
#     data_row_count = 0
#     data_started = False
...
# def main():
#     apo_folder = 'C:\\Users\\kaixiang\\Downloads\\Single_Color_Somas'  # 请将此路径替换为您的 .apo 文件夹路径
#     for filename in os.listdir(apo_folder):
#         if filename.endswith('.apo'):
#             file_path = os.path.join(apo_folder, filename)
#             try:
#                 row_count = count_data_rows_in_apo(file_path)
